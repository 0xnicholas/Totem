...
import type { FeishuCredsStore } from './creds-store.js';
 ...
 * (ADR-0004): one method, everything else hidden behind it.
 */
...
/**
 * The ADR-0004 deep module: the entire OAuth token lifecycle behind
 * `getValidAccessToken` —
 *
 ...
 *   single in-flight refresh promise — exactly one refresh hits Feishu;
...
export class TokenManager implements TokenProvider {
  ...
  async getValidAccessToken(connectionId: string): Promise<string> {
    ...
    // no point touching the token store or Feishu until re-authorization.
  ...
  private async doRefresh(connectionId: string, stored: StoredTokens): Promise<string> {
    ...
    if (!creds) {
      throw new ActionError(
        'auth_expired',
        `Tenant "${stored.tenantId}" has no Feishu credentials configured; refresh impossible`,
      );
    }
    ...
    try {
      ...
      if (err instanceof FeishuApiError) {
        if (err.httpStatus === 429) {
          throw new ActionError('rate_limited', `Feishu rate limited during refresh: ${err.message}`);
        }
        if (err.invalidGrant) {
          ...
          throw new ActionError('auth_expired', `Feishu rejected the refresh token: ${err.message}`);
        }
      }
      throw new ActionError('upstream_error', `Feishu token refresh failed: ${errorMessage(err)}`);
    }
  }
}
