 ...
 * 3. validate `args` against the action's input schema
 * 4. dispatch through the connection's connector (`execute`)
 * 5. validate the handler output against the action's output schema
 *
 * Governance (schema validation, orchestration-level error mapping; later
 * allowlist and audit) lives here per ADR-0003 — the connector only
 * translates.
 */
export class ActionExecutor {
  ...
  constructor(registry: ActionRegistry, connections: ConnectionStore) {
  ...
  async executeAction(
    ...
    try {
      ...
      if (isActionError(err)) return { ok: false, error: err };
...
export function createActionExecutor(config: {
  ...
  return new ActionExecutor(registry, new ConnectionStore(config.connections));
}
