...
describe.runIf(hasDb)('migrations', () => {
  ...
  it('applies all pending migrations and creates every migration table with tenant_id and foreign keys', async () => {
    ...
    for (const table of ['tenants', 'connections', 'api_keys', 'allowlists', 'audit_logs', 'tokens', 'feishu_credentials', 'dingtalk_credentials', 'wecom_credentials', 'schema_migrations']) {
